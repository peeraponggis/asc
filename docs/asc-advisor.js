...
(function (global) {
    ...
    function normalize(input) {
        ...
        if (b) {
            ...
            return {
                ...
                /* ระบบป้องกันฟ้าผ่าของอาคาร · ไฟล์เก่าไม่มีก้อนนี้ จึงเป็น {} แล้วกฎจะเตือนเอง */
                lps: s3.lightning_protection || {},
...

    {
        /* วสท. ข้อ 3.6.2 (3) และตารางที่ 3.2 หน้า 32-33 · อุปกรณ์ป้องกันไฟกระชาก
           ...
           เดิมกฎนี้ตอบได้แค่ "ต้องยืนยัน class เอง" เพราะโปรแกรมไม่รู้เรื่องระบบล่อฟ้าของอาคาร
           ตอนนี้หน้าออกแบบถามแล้วสองข้อ (มีล่อฟ้าไหม / รักษาระยะแยก S ได้ไหม)
           จึงตัดสินจากตารางที่ 3.2 ได้จริง

           ที่ยังไม่เปลี่ยนคือหลักการ ข้อมูลไม่พอต้องเตือน ไม่ใช่เดาเป็น class II
           เพราะ class II ผิดในสองในสามเคสของตาราง */
        id: 'spd',
        run(d) {
            ...
            if (String(p.ac_spd_class) !== want.ac) wrong.push('ฝั่งไฟสลับควรเป็น class ' + want.ac);

            if (wrong.length) return { level: 'error', title: 'ชนิดของ SPD ไม่ตรงกับตารางที่ 3.2',
                detail: want.label + ' ตารางที่ 3.2 จึงกำหนดให้ใช้ SPD ฝั่งไฟตรง class ' + want.dc +
                        ' และฝั่งไฟสลับ class ' + want.ac + ' แต่ในรายการวัสดุไม่ตรง ' +
                        'ของที่สั่งจะป้องกันไม่ได้ตามที่มาตรฐานกำหนด',
                evidence: ev + ' · ' + want.label,
                fix: wrong.concat(['กด Generate BOM ใหม่หลังแก้ข้อมูลระบบป้องกันฟ้าผ่า ระบบจะเลือกชนิดให้เอง']),
                kb: 'eit-lps', eit: CL };

            return { level: 'ok', title: 'ชนิดของ SPD ตรงกับตารางที่ 3.2',
                detail: want.label + ' จึงใช้ SPD ฝั่งไฟตรง class ' + want.dc +
                        ' และฝั่งไฟสลับ class ' + want.ac +
                        (want.scenario === 's-not-ok'
                            ? ' · กรณีนี้ยังต้องต่อตัวนำโดยตรงเข้าระบบล่อฟ้าด้วย ไม่ใช่ต่อผ่าน SPD อย่างเดียว'
                            : ''),
                evidence: ev, kb: 'eit-lps', eit: CL };
        }
    },

    {
        /* วสท. ข้อ 3.6.2 (1) หน้า 32 · การประสานศักย์ให้เท่ากันทางฟ้าผ่า

           ตัวบทแยกทางเดินไว้สองทางชัดเจน และเป็นคนละวิธีกันจริง ๆ ไม่ใช่แค่ถ้อยคำ
               รักษาระยะแยกได้    ต่อผ่านอุปกรณ์ป้องกันเสิร์จ "เท่านั้น"
               รักษาระยะแยกไม่ได้  ต่อตัวนำ "โดยตรง" เข้าระบบล่อฟ้า "และ" ต่อผ่าน SPD

           ขนาดสายต่อประสานมาจากภาคผนวก จ ซึ่งเป็นข้อแนะนำ ไม่ใช่ข้อบังคับ
           ระดับผลตรวจของส่วนนั้นจึงเป็น warn ไม่ใช่ error */
        id: 'lps-bonding',
        run(d) {
            const L = d.lps || {};
            const ext = String(L.external || '').toLowerCase();
            const CL = eitClause('lpsBonding') || '3.6.2 (1)';

            if (!ext || ext === 'unknown') return { level: 'warn',
                title: 'ยังไม่ได้ระบุว่าอาคารมีระบบป้องกันฟ้าผ่าหรือไม่',
                detail: 'ข้อ 3.6.1 บอกว่าการติดตั้ง PV บนหลังคาเองไม่ได้ทำให้ต้องติดระบบป้องกันฟ้าผ่าเพิ่ม ' +
                        'แต่ถ้าอาคารมีอยู่แล้ว ระบบของ PV ต้องรวมเข้ากับของเดิมอย่างเหมาะสม ' +
                        'และเป็นตัวตัดสินชนิดของ SPD กับขนาดสายต่อประสานศักย์ทั้งงาน',
                evidence: 'ยังไม่มีข้อมูลระบบป้องกันฟ้าผ่าใน DB2 (step3_design_and_layout.lightning_protection)',
                fix: ['ไปที่ขั้นที่ 1 เลือกในช่อง "ระบบป้องกันฟ้าผ่าของอาคาร"',
                      'ถ้าไม่แน่ใจ ให้ดูว่าบนหลังคามีแท่งล่อฟ้าและสายตัวนำลงดินหรือไม่'],
                kb: 'eit-lps', eit: eitClause('lps') || '3.6.1' };

            const bond = d.bom && d.bom.protection ? num(d.bom.protection.bonding_sqmm) : null;
            const wantBond = (global.AscEIT && global.AscEIT.bondingSqmmFor) ? global.AscEIT.bondingSqmmFor(L) : null;

            if (ext === 'none') {
                const ev = 'อาคารไม่มีระบบป้องกันฟ้าผ่าภายนอก · สายต่อประสานศักย์ ' +
                           (bond ? bond + ' ตร.มม.' : 'ยังไม่มีในรายการวัสดุ');
                if (wantBond && bond && bond < wantBond) return { level: 'warn',
                    title: 'สายต่อประสานศักย์เล็กกว่าที่ภาคผนวก จ แนะนำ',
                    detail: 'รูปที่ จ.3 แนะนำสายทองแดง ' + wantBond + ' ตร.มม. สำหรับหลังคาที่ไม่มีระบบป้องกันฟ้าผ่า ' +
                            'ภาคผนวก จ เป็นข้อแนะนำ ไม่ใช่ข้อบังคับ แต่เป็นตัวเลขเดียวที่เล่มนี้ให้ไว้',
                    evidence: ev, fix: ['ใช้สายต่อประสานศักย์อย่างน้อย ' + wantBond + ' ตร.มม.'],
                    kb: 'eit-lps', eit: CL };
                return { level: 'ok', title: 'อาคารไม่มีระบบป้องกันฟ้าผ่า ใช้การประสานศักย์แบบพื้นฐาน',
                    detail: 'ข้อ 3.6.1 ระบุว่าการติดตั้ง PV บนหลังคามีความเสี่ยงถูกฟ้าผ่าโดยตรงค่อนข้างต่ำ ' +
                            'จึงไม่จำเป็นต้องติดระบบป้องกันฟ้าผ่าเพิ่ม เว้นแต่ลักษณะทางกายภาพของอาคาร ' +
                            'เปลี่ยนแปลงไปอย่างมากจากการติดตั้ง ซึ่งต้องให้วิศวกรประเมินเอง',
                    evidence: ev, kb: 'eit-lps', eit: eitClause('lps') || '3.6.1' };
            }

            /* ext === 'yes' */
            const sep = String(L.separation || '').toLowerCase();
            if (sep !== 'ok' && sep !== 'not_ok') return { level: 'warn',
                title: 'อาคารมีระบบป้องกันฟ้าผ่า แต่ยังไม่ได้ประเมินระยะการแยก S',
                detail: 'ข้อ 3.6.2 (1) แยกวิธีประสานศักย์เป็นสองทางตามระยะการแยก ' +
                        'รักษาได้ให้ต่อผ่านอุปกรณ์ป้องกันเสิร์จเท่านั้น ' +
                        'รักษาไม่ได้ต้องต่อตัวนำโดยตรงเข้าระบบป้องกันฟ้าผ่าและต่อผ่านอุปกรณ์ป้องกันเสิร์จด้วย ' +
                        'สองแบบนี้ใช้ของคนละชุดและเดินสายคนละแบบ',
                evidence: 'อาคารมีระบบป้องกันฟ้าผ่าภายนอก แต่ช่องระยะการแยกยังเป็น "ยังไม่ประเมิน"',
                fix: ['ประเมินระยะการแยกตามมาตรฐานการป้องกันฟ้าผ่าของ วสท. ซึ่งเป็นคนละเล่มกับเล่มนี้',
                      'พื้นที่จำกัดบนหลังคาส่วนใหญ่รักษาระยะแยกไม่ได้ ให้ตรวจหน้างานก่อนสรุป'],
                kb: 'eit-lps', eit: CL };

            const ev2 = (sep === 'ok' ? 'รักษาระยะการแยก S ได้' : 'รักษาระยะการแยก S ไม่ได้') +
                        ' · สายต่อประสานศักย์ ' + (bond ? bond + ' ตร.มม.' : 'ยังไม่มีในรายการวัสดุ');

            if (wantBond && bond && bond < wantBond) return { level: 'warn',
                title: 'สายต่อประสานศักย์เล็กกว่าที่ภาคผนวก จ แนะนำ',
                detail: 'รูปที่ จ.4 แนะนำสายทองแดง ' + wantBond + ' ตร.มม. สำหรับหลังคาที่มีระบบป้องกันฟ้าผ่า ' +
                        'ซึ่งใหญ่กว่ากรณีไม่มีล่อฟ้าถึงสี่เท่า เพราะต้องรับกระแสฟ้าผ่าบางส่วนได้',
                evidence: ev2, fix: ['ใช้สายต่อประสานศักย์อย่างน้อย ' + wantBond + ' ตร.มม.'],
                kb: 'eit-lps', eit: CL };

            if (sep === 'not_ok') return { level: 'warn',
                title: 'รักษาระยะการแยกไม่ได้ ต้องต่อตัวนำโดยตรงเข้าระบบป้องกันฟ้าผ่า',
                detail: 'ข้อ 3.6.2 (1) กำหนดว่ากรณีระยะแยกไม่ได้ ให้ใช้วิธีต่อตัวนำโดยตรงเข้ากับระบบป้องกันฟ้าผ่า ' +
                        '"และ" ต่อผ่านอุปกรณ์ป้องกันเสิร์จ ไม่ใช่อย่างใดอย่างหนึ่ง ' +
                        'ภาคผนวก จ.1 อธิบายเหตุผลว่าเพื่อไม่ให้กระแสฟ้าผ่าบางส่วนไหลผ่านส่วนโลหะของ PV array ' +
                        'ข้อนี้เป็นงานหน้างานที่โปรแกรมตรวจให้ไม่ได้ ต้องยืนยันในแบบและตอนตรวจรับ',
                evidence: ev2,
                fix: ['ต่อส่วนโลหะของ PV array เข้ากับระบบป้องกันฟ้าผ่าโดยตรงด้วยสาย ' + (wantBond || 16) + ' ตร.มม.',
                      'ติดตั้ง SPD ตามตารางที่ 3.2 ควบคู่ไปด้วย ไม่ใช่แทนกัน',
                      'เขียนจุดต่อประสานลงในแบบ เพื่อให้ตรวจรับหน้างานได้'],
                kb: 'eit-lps', eit: CL };

            return { level: 'ok', title: 'รักษาระยะการแยกได้ ใช้การต่อผ่านอุปกรณ์ป้องกันเสิร์จ',
                detail: 'ข้อ 3.6.2 (1) กำหนดว่ากรณีระยะแยกได้ ให้ใช้วิธีต่อผ่านอุปกรณ์ป้องกันเสิร์จเท่านั้น ' +
                        'ห้ามต่อตัวนำโดยตรง เพราะจะทำให้กระแสฟ้าผ่าไหลเข้าส่วนโลหะของ PV array โดยไม่จำเป็น',
                evidence: ev2, kb: 'eit-lps', eit: CL };
        }
    },

    {
        /* ระยะการแยก S · เทียบค่าที่คำนวณได้กับระยะจริงบนผัง

           สูตรและค่าสัมประสิทธิ์อยู่ใน asc-lps.js พร้อมที่มาครบ
           กฎนี้ไม่คำนวณเองเลย เรียกโมดูลอย่างเดียว จะได้มีที่มาที่เดียว

           ข้อที่สำคัญที่สุดคือกรณีที่ **ผู้ใช้ตอบขัดกับผลคำนวณ**
           เพราะคำตอบช่องนั้นไปตัดสินชนิดของ SPD ตามตารางที่ 3.2
           ตอบว่ารักษาได้ทั้งที่เรขาคณิตบอกว่าไม่ได้ จะสั่ง SPD ผิดชนิดทั้งงาน */
        id: 'lps-separation',
        run(d) {
            const LP = global.AscLps;
            if (!LP || !LP.separation) return null;          // ไม่ได้โหลดโมดูล ต้องเงียบ
            const L = d.lps || {};
            if (String(L.external || '').toLowerCase() !== 'yes') return null;

            const CL = eitClause('lpsBonding') || '3.6.2 (1)';
            const SRC = 'สูตรและค่าสัมประสิทธิ์จากเอกสารบรรยาย ลือชัย ทองนิล (2566) ' +
                        'ซึ่งอ้างมาตรฐานการป้องกันฟ้าผ่าของ วสท.';

            const need = LP.separation({
                lpl: L.lpl, material: L.separation_material,
                downConductors: L.down_conductor_count, length: L.conductor_length_m
            });

            if (!need.ok) return { level: 'info',
                title: 'ยังคำนวณระยะการแยก S ให้ไม่ได้',
                detail: 'ต้องการอีก ' + need.missing.join(' · ') + ' จึงจะคำนวณ s ได้ ' +
                        'เมื่อคำนวณได้แล้วโปรแกรมจะเทียบกับระยะจริงบนผังให้เอง ' +
                        'แทนที่จะให้ตอบเองว่ารักษาระยะแยกได้หรือไม่',
                evidence: 'อาคารมีระบบป้องกันฟ้าผ่า แต่ข้อมูลสำหรับคำนวณ s ยังไม่ครบ',
                fix: ['กรอกช่องคำนวณระยะการแยก S ในการ์ดระบบป้องกันฟ้าผ่า ขั้นที่ 1',
                      'ชั้นการป้องกัน (LPL) ต้องให้วิศวกรเลือกเอง โปรแกรมประเมินความเสี่ยงให้ไม่ได้'],
                kb: 'eit-lps', eit: CL };

            const rods = Array.isArray(L.air_terminals) ? L.air_terminals : [];
            const said = String(L.separation || '').toLowerCase();
            const evBase = 's ที่ต้องการ ' + need.s + ' ม. · ' + need.working + ' · ' + SRC;

            if (!rods.length) return { level: 'info',
                title: 'คำนวณระยะการแยกได้แล้ว แต่ยังไม่มีเสาล่อฟ้าบนผัง',
                detail: 'ต้องเว้นระยะอย่างน้อย ' + need.s + ' ม. ระหว่างส่วนโลหะของ PV array ' +
                        'กับตัวนำของระบบป้องกันฟ้าผ่า ยังไม่ได้ปักเสาล่อฟ้าลงผัง ' +
                        'จึงเทียบกับระยะจริงให้ไม่ได้ ต้องวัดหน้างานเอง',
                evidence: evBase,
                fix: ['ปักตำแหน่งเสาล่อฟ้าลงบนผังในขั้นที่ 1 แล้วโปรแกรมจะวัดระยะให้',
                      'หรือวัดระยะจริงหน้างานแล้วเทียบกับ ' + need.s + ' ม. ด้วยตัวเอง'],
                kb: 'eit-lps', eit: CL };

            /* หาเสาที่แย่ที่สุด ระยะแยกต้องผ่านทุกจุด ไม่ใช่ผ่านเฉลี่ย */
            let worstGap = null;
            rods.forEach(r => {
                const g = r && r.result ? num(r.result.gap) : null;
                if (g === null) return;
                if (worstGap === null || g < worstGap) worstGap = g;
            });

            if (worstGap === null) return { level: 'info',
                title: 'มีเสาล่อฟ้าบนผัง แต่วัดระยะถึงพื้นที่วางแผงไม่ได้',
                detail: 'อาจยังไม่ได้วาดผืนหลังคาหรือกรอบพื้นที่วางแผง จึงไม่มีอะไรให้วัดระยะถึง',
                evidence: evBase, kb: 'eit-lps', eit: CL };

            const computed = worstGap >= need.s ? 'ok' : 'not_ok';
            const ev = evBase + ' · ระยะจริงที่แคบที่สุด ' + worstGap + ' ม. จากเสา ' + rods.length + ' ต้น';

            /* คำตอบของผู้ใช้ขัดกับผลคำนวณ เป็นข้อที่ต้องดังที่สุด */
            if ((said === 'ok' || said === 'not_ok') && said !== computed) return { level: 'error',
                title: 'คำตอบเรื่องระยะการแยกขัดกับผลคำนวณจากผัง',
                detail: 'ในแบบระบุว่า' + (said === 'ok' ? 'รักษาระยะแยกได้' : 'รักษาระยะแยกไม่ได้') +
                        ' แต่คำนวณจากผังแล้วได้ว่า' + (computed === 'ok' ? 'รักษาได้' : 'รักษาไม่ได้') + ' ' +
                        'คำตอบนี้เป็นตัวตัดสินชนิดของ SPD ตามตารางที่ 3.2 ถ้าตอบผิดจะสั่งของผิดทั้งงาน ' +
                        'ต้องหาข้อสรุปให้ตรงกันก่อน ระยะที่โปรแกรมวัดเป็นระยะถึงขอบพื้นที่วางแผง ' +
                        'ไม่ใช่ระยะถึงชิ้นโลหะที่ใกล้ที่สุดจริง ๆ จึงอาจต่างจากหน้างานได้',
                evidence: ev,
                fix: ['ตรวจตำแหน่งเสาล่อฟ้าบนผังว่าตรงกับหน้างานหรือไม่',
                      'ตรวจความยาวตัวนำ l ที่กรอกไว้ว่าวัดจากจุดที่ถูกต้อง',
                      'ถ้าผังถูกแล้ว ให้แก้คำตอบในช่องรักษาระยะแยกให้ตรงกับผลคำนวณ'],
                kb: 'eit-lps', eit: CL };

            if (computed === 'not_ok') return { level: 'warn',
                title: 'ระยะจริงบนผังน้อยกว่าระยะการแยกที่ต้องการ',
                detail: 'ต้องเว้นอย่างน้อย ' + need.s + ' ม. แต่ระยะที่แคบที่สุดบนผังคือ ' + worstGap + ' ม. ' +
                        'กรณีนี้รักษาระยะแยกไม่ได้ ต้องต่อตัวนำโดยตรงเข้าระบบป้องกันฟ้าผ่า ' +
                        'และใช้ SPD class I ทั้งสองฝั่งตามตารางที่ 3.2 ' +
                        'เป็นเรื่องปกติของหลังคาที่พื้นที่จำกัด ไม่ใช่ความผิดพลาดในการออกแบบ',
                evidence: ev,
                fix: ['ตั้งช่องรักษาระยะแยกเป็น "รักษาไม่ได้" แล้วกด Generate BOM ใหม่',
                      'หรือย้ายเสาล่อฟ้าให้ห่างจากพื้นที่วางแผงอย่างน้อย ' + need.s + ' ม. ถ้าพื้นที่พอ'],
                kb: 'eit-lps', eit: CL };

            return { level: 'ok',
                title: 'ระยะจริงบนผังไม่น้อยกว่าระยะการแยกที่ต้องการ',
                detail: 'ต้องเว้นอย่างน้อย ' + need.s + ' ม. ระยะที่แคบที่สุดบนผังคือ ' + worstGap + ' ม. ' +
                        'จึงรักษาระยะแยกได้ และให้ต่อประสานผ่านอุปกรณ์ป้องกันเสิร์จเท่านั้น ' +
                        'ห้ามต่อตัวนำโดยตรง ตามข้อ 3.6.2 (1)',
                evidence: ev, kb: 'eit-lps', eit: CL };
        }
    },

    {
        /* เขตป้องกันของเสาล่อฟ้า · วิธีมุมป้องกัน ตารางที่ 4.1

           ตารางที่มีถูกตัดมาแค่ h = 1-17 ม. เกินกว่านั้นต้องบอกว่าคำนวณไม่ได้
           ห้ามประมาณต่อ เพราะเส้นโค้งมุมป้องกันไม่เป็นเส้นตรง */
        id: 'lps-zone',
        run(d) {
            const LP = global.AscLps;
            if (!LP || !LP.protectionAngle) return null;
            const L = d.lps || {};
            const rods = Array.isArray(L.air_terminals) ? L.air_terminals : [];
            if (!rods.length) return null;                   // ไม่มีเสา ไม่มีอะไรให้ตรวจ

            const CL = eitClause('lps') || '3.6.1';
            const over = [], zones = [];
            rods.forEach((r, i) => {
                const z = r && r.result ? r.result.zone : null;
                if (!z) return;
                if (z.outOfRange) over.push('ต้นที่ ' + (i + 1) + ' สูง ' + z.h + ' ม.');
                else zones.push('ต้นที่ ' + (i + 1) + ' รัศมี ' + z.radius + ' ม. (มุม ' + z.alpha + ' องศา)');
            });

            if (over.length) return { level: 'warn',
                title: 'มีเสาล่อฟ้าที่สูงเกินช่วงของตารางมุมป้องกันที่มี',
                detail: 'ตารางที่ 4.1 ที่ใช้อยู่ครอบคลุมความสูงแท่งล่อฟ้า 1 ถึง ' + (LP.ANGLE_H_MAX || 17) +
                        ' ม. เท่านั้น เพราะเป็นส่วนที่ตัดมาจากเอกสารอ้างอิง ' +
                        'เสาที่สูงกว่านี้จึงคำนวณเขตป้องกันด้วยวิธีมุมป้องกันให้ไม่ได้ ' +
                        'ต้องเปิดตารางฉบับเต็ม หรือใช้วิธีทรงกลมกลิ้งซึ่งใช้ได้ทุกความสูง',
                evidence: over.join(' · '),
                fix: ['ใช้วิธีทรงกลมกลิ้งรัศมีตามชั้นการป้องกันแทน (I 20 · II 30 · III 45 · IV 60 ม.)',
                      'หรือหาตารางที่ 4.1 ฉบับเต็มจากมาตรฐานการป้องกันฟ้าผ่าของ วสท.'],
                kb: 'eit-lps', eit: CL };

            if (!zones.length) return null;
            return { level: 'info',
                title: 'เขตป้องกันของเสาล่อฟ้าบนผัง',
                detail: 'คำนวณด้วยวิธีมุมป้องกัน โดยวัดความสูงแท่งเทียบกับผิวหลังคาที่จะป้องกัน ' +
                        'ไม่ใช่เทียบระดับพื้นดิน · เขตป้องกันนี้เป็นเพียงรัศมีบนระนาบหลังคา ' +
                        'ยังไม่ได้ตรวจว่าแผงทุกแผ่นอยู่ในเขตหรือไม่ ต้องดูจากวงบนผังเอง',
                evidence: zones.join(' · '),
                kb: 'eit-lps', eit: CL };
        }
    },
    ...
    {
        /* วสท. ภาคผนวก จ.2 หน้า 120 · การต่อลงดินของระบบป้องกันฟ้าผ่า

           สองค่าที่เล่มให้ไว้ตรง ๆ  ตัวนำลงดินไม่น้อยกว่า 50 ตร.มม. (จ.2.1)
                                    ความต้านทานดินรวมไม่เกิน 10 โอห์ม (จ.2.2)

           ทั้งสองเป็นค่าที่วัดหรือออกแบบหน้างาน โปรแกรมคำนวณเองไม่ได้
           ทำได้แค่เทียบกับเกณฑ์เมื่อผู้ใช้กรอกมา ไม่กรอกก็ต้องเงียบ ไม่ใช่เดาว่าผ่าน */
        id: 'lps-earth',
        run(d) {
            const L = d.lps || {};
            if (String(L.external || '').toLowerCase() !== 'yes') return null;   // ไม่มีล่อฟ้า ไม่มีอะไรให้ตรวจ

            const CL = eitClause('lpsEarth') || 'ภาคผนวก จ.2';
            const E = global.AscEIT || {};
            const minSq = E.LPS_DOWN_CONDUCTOR_MIN_SQMM || 50;
            const maxOhm = E.LPS_EARTH_RESISTANCE_MAX_OHM || 10;
            const sq  = num(L.down_conductor_sqmm);
            const ohm = num(L.earth_resistance_ohm);

            if (!sq && !ohm) return { level: 'info',
                title: 'ยังไม่มีผลวัดระบบรากสายดินของระบบป้องกันฟ้าผ่า',
                detail: 'ภาคผนวก จ.2 แนะนำตัวนำลงดินแต่ละตัวไม่น้อยกว่า ' + minSq + ' ตร.มม. ' +
                        'และความต้านทานดินรวมไม่เกิน ' + maxOhm + ' โอห์ม ก่อนต่อประสานกับระบบสาธารณูปโภคอื่น ' +
                        'สองค่านี้วัดหรือออกแบบหน้างาน โปรแกรมคำนวณให้ไม่ได้',
                evidence: 'อาคารมีระบบป้องกันฟ้าผ่า แต่ยังไม่ได้กรอกขนาดตัวนำลงดินและผลวัดความต้านทานดิน',
                fix: ['วัดความต้านทานดินของระบบป้องกันฟ้าผ่าแล้วกรอกในขั้นที่ 1',
                      'กรอกขนาดตัวนำลงดินที่ใช้จริง เพื่อให้ผลตรวจและเอกสารส่งมอบมีหลักฐาน'],
                kb: 'eit-lps', eit: CL };

            const bad = [], ev = [];
            if (sq)  { ev.push('ตัวนำลงดิน ' + sq + ' ตร.มม.');  if (sq < minSq)   bad.push('ตัวนำลงดินเล็กกว่า ' + minSq + ' ตร.มม.'); }
            if (ohm) { ev.push('ความต้านทานดิน ' + ohm + ' โอห์ม'); if (ohm > maxOhm) bad.push('ความต้านทานดินเกิน ' + maxOhm + ' โอห์ม'); }

            if (bad.length) return { level: 'warn',
                title: 'ระบบรากสายดินของระบบป้องกันฟ้าผ่าไม่ถึงเกณฑ์ที่แนะนำ',
                detail: bad.join(' และ ') + ' · ภาคผนวก จ.2 เป็นข้อแนะนำ ไม่ใช่ข้อบังคับ ' +
                        'แต่เป็นเกณฑ์เดียวที่เล่มนี้ให้ไว้ และมีผลโดยตรงต่อแรงดันช่วงก้าวกับแรงดันสัมผัส ' +
                        'รวมถึงความเสี่ยงวาบไฟด้านข้างไปยังส่วนโลหะที่อยู่ใกล้',
                evidence: ev.join(' · '),
                fix: (sq && sq < minSq ? ['เปลี่ยนตัวนำลงดินเป็นขนาดไม่น้อยกว่า ' + minSq + ' ตร.มม.'] : [])
                     .concat(ohm && ohm > maxOhm ? ['เพิ่มแท่งรากสายดินหรือปรับปรุงระบบรากสายดินให้ความต้านทานลดลงเหลือไม่เกิน ' + maxOhm + ' โอห์ม'] : []),
                kb: 'eit-lps', eit: CL };

            return { level: 'ok', title: 'ระบบรากสายดินของระบบป้องกันฟ้าผ่าอยู่ในเกณฑ์',
                detail: 'ผ่านเกณฑ์ที่ภาคผนวก จ.2 แนะนำไว้ ' +
                        (sq ? 'ตัวนำลงดินไม่น้อยกว่า ' + minSq + ' ตร.มม. ' : '') +
                        (ohm ? 'และความต้านทานดินไม่เกิน ' + maxOhm + ' โอห์ม' : ''),
                evidence: ev.join(' · '), kb: 'eit-lps', eit: CL };
        }
    },
    ...
    {
        /* แบตเตอรี่กับอินเวอร์เตอร์ต้องต่อกันได้จริง

           ข้อนี้ไม่ได้มาจากเล่ม วสท. โดยตรง แต่เป็นความผิดพลาดที่เงียบที่สุด
           แบบหนึ่ง เลือกแบตแรงดันต่ำ 48 V ไปคู่กับอินเวอร์เตอร์แรงดันสูง 100-700 V
           โปรแกรมก็คำนวณพลังงานและคืนทุนจนจบ ออกใบเสนอราคาได้ตามปกติ
           กว่าจะรู้ก็ตอนของถึงหน้างาน

           ตรรกะทั้งหมดอยู่ใน asc-batmatch.js ชุดเดียวกับที่ตัวเลือกอุปกรณ์ใช้
           จะได้ไม่มีสองมาตรฐานที่เพี้ยนออกจากกัน ถ้าโมดูลไม่ถูกโหลด คืน null เงียบ ๆ */
        id: 'bat-inverter',
        run(d) {
            const BM = global.AscBatMatch;
            if (!BM) return null;
            ...
            const bName = (bat.Manufacturer ? bat.Manufacturer + ' ' : '') + (bat.Model_Name || bat.model);
            const ev = bName + ' + ' + iName + ' · ' + (BM.LABEL[r.level] || r.level);

            if (r.level === 'no') return { level: 'error',
                title: 'แบตเตอรี่กับอินเวอร์เตอร์ต่อกันไม่ได้',
                detail: r.why + ' ถ้าสั่งของตามนี้ จะต่อใช้งานไม่ได้จริงที่หน้างาน ' +
                        'แต่ตัวเลขในรายงานจะคำนวณออกมาเหมือนใช้ได้ทุกอย่าง',
                evidence: ev,
                fix: ['เปลี่ยนแบตเตอรี่เป็นชนิดแรงดันเดียวกับที่อินเวอร์เตอร์รองรับ',
                      'หรือเปลี่ยนอินเวอร์เตอร์เป็นรุ่นที่รับแบตเตอรี่ชุดนี้ได้',
                      'ดูกลุ่ม ตรงตามเอกสาร ในตัวเลือกแบตเตอรี่ที่หน้าออกแบบ'],
                kb: 'battery-match' };
            ...
            if (r.level === 'partial') {
                ...
                const fixes = [];
                if (m && m.max >= m.min) {
                    fixes.push('เลือกรุ่นที่มี ' + m.min + '-' + m.max + ' โมดูล ' +
                               'หรือถอดโมดูลออกให้เหลือไม่เกิน ' + m.max + ' โมดูล แล้วเขียนจำนวนที่ใช้จริงไว้ในแบบ');
                    fixes.push('ความจุที่ได้จริงจะลดลงตามจำนวนโมดูลที่เหลือ ต้องคิดใหม่ก่อนเสนอราคา');
                } else {
                    fixes.push('ระบุจำนวนโมดูลที่ทำให้แรงดันอยู่ในช่วงของอินเวอร์เตอร์ แล้วเขียนไว้ในแบบ');
                }
                return { level: 'warn',
                    title: 'แบตเตอรี่เข้ากับอินเวอร์เตอร์ได้ไม่เต็มชุด',
                    detail: r.why + ' ต้องระบุจำนวนโมดูลที่ใช้จริงให้ชัดในแบบ ' +
                            'ไม่ใช่สั่งตามความจุรวมของตู้เต็ม',
                    evidence: ev + (m ? ' · ใช้ได้ ' + m.min + '-' + m.max + ' โมดูล จากที่ตู้มี ' + m.have : ''),
                    fix: fixes, kb: 'battery-match' };
            }

            if (r.level === 'doc') return { level: 'ok',
                title: 'แบตเตอรี่กับอินเวอร์เตอร์ตรงตามเอกสารผู้ผลิต',
                detail: r.why + (r.option ? ' · หน้างานต้องตั้ง Battery Option เป็น ' + r.option : ''),
                evidence: ev, kb: 'battery-match' };

            if (r.level === 'volt') return { level: 'info',
                title: 'แบตเตอรี่แรงดันเข้ากันได้ แต่เอกสารไม่ได้ยืนยัน',
                detail: r.why + ' ดาต้าชีตของทั้งสองฝั่งไม่ได้ระบุอีกฝั่งไว้ ' +
                        'จึงยันยืนไม่ได้ว่า BMS คุยกันรู้เรื่อง ต้องเช็คกับผู้ผลิตก่อนสั่งของ',
                evidence: ev,
                fix: ['ขอ compatibility list ฉบับล่าสุดจากผู้ผลิตอินเวอร์เตอร์ แล้วเติมลงดาต้าชีตในคลัง'],
                kb: 'battery-match' };

            return { level: 'info',
                title: 'ตรวจการจับคู่แบตเตอรี่กับอินเวอร์เตอร์ไม่ได้',
                detail: r.why + ' จึงยังฟันธงไม่ได้ว่าคู่นี้ใช้ด้วยกันได้หรือไม่',
                evidence: ev,
                fix: ['เติมช่วงแรงดันแบตเตอรี่ในดาต้าชีตของอุปกรณ์ทั้งสองตัว'],
                kb: 'battery-match' };
        }
    },

    {
        /* กำลังชาร์จและคายประจุที่ระบบทำได้จริง

           ข้อนี้ไม่ใช่ความปลอดภัย แต่เป็นตัวเลขที่ไปโผล่ในข้อเสนอที่ลูกค้าเซ็น
           อินเวอร์เตอร์ MG10RL พิกัดไฟสำรอง 10 kW แต่แบต MGL060 หนึ่งก้อน
           จ่ายต่อเนื่องได้ 3.07 kW ไฟสำรองจริงคือ 3.07 kW ไม่ใช่ 10 kW
           BMS จะจำกัดให้เอง โปรแกรมไม่ได้ผิดอะไร แต่ตัวเลขในใบเสนอราคาจะเกินจริง

           เตือนเฉพาะตอนที่แบตจ่ายไม่ถึง **พิกัดไฟสำรอง** ของอินเวอร์เตอร์
           เพราะนั่นคือเลขที่เขียนลงเอกสาร ส่วนการที่แบตเล็กกว่าพิกัดชาร์จสูงสุด
           เป็นเรื่องปกติและตั้งใจ จึงเป็นแค่ข้อสังเกต ไม่ใช่คำเตือน */
        id: 'bat-power',
        run(d) {
            const BM = global.AscBatMatch;
            ...
            const kw = BM.fmtKw;
            const bq = r.batQty > 1 ? ' (' + r.batQty + ' ชุด)' : '';
            const parts = [];
            if (r.batDis) parts.push('แบตคายประจุได้ ' + kw(r.batDis) + ' kW' + bq);
            if (r.invDis) parts.push('อินเวอร์เตอร์รับได้ ' + kw(r.invDis) + ' kW');
            if (r.backup) parts.push('พิกัดไฟสำรอง ' + kw(r.backup) + ' kW');
            const ev = parts.join(' · ') + (r.derived ? ' · กำลังของแบตคำนวณจากกระแสคูณแรงดันระบุ' : '');

            if (r.backupShort) return { level: 'warn',
                title: 'แบตเตอรี่จ่ายไฟไม่ถึงพิกัดไฟสำรองของอินเวอร์เตอร์',
                detail: 'อินเวอร์เตอร์พิกัดไฟสำรอง ' + kw(r.backup) + ' kW แต่แบตเตอรี่ชุดนี้' +
                        'คายประจุต่อเนื่องได้ ' + kw(r.batDis) + ' kW ' +
                        'ไฟสำรองที่ใช้ได้จริงคือ ' + kw(r.batDis) + ' kW ไม่ใช่ ' + kw(r.backup) + ' kW ' +
                        'BMS จะจำกัดให้เอง ระบบไม่พัง แต่ตัวเลขในข้อเสนอจะเกินจริงถ้าเขียนตามพิกัดอินเวอร์เตอร์',
                evidence: ev,
                fix: ['เพิ่มจำนวนชุดแบตเตอรี่ให้กำลังคายประจุรวมถึง ' + kw(r.backup) + ' kW',
                      'หรือเขียนกำลังไฟสำรองในข้อเสนอเป็น ' + kw(r.batDis) + ' kW ตามที่ทำได้จริง',
                      'ถ้าโหลดสำรองไม่ได้ต้องการเต็มพิกัดอยู่แล้ว ให้ระบุโหลดที่จะสำรองไว้ในแบบ'],
                kb: 'battery-match' };

            if (r.disLimitedByBattery || r.chgLimitedByBattery) return { level: 'info',
                title: 'กำลังชาร์จและคายประจุถูกจำกัดที่แบตเตอรี่ ไม่ใช่ที่อินเวอร์เตอร์',
                detail: 'คายประจุได้จริง ' + kw(r.disLimit) + ' kW และชาร์จได้จริง ' + kw(r.chgLimit) + ' kW ' +
                        'เป็นเรื่องปกติที่แบตเล็กกว่าพิกัดอินเวอร์เตอร์ แต่ต้องใช้ตัวเลขนี้ตอนคิดเวลาสำรองไฟ ' +
                        'ไม่ใช่พิกัดของอินเวอร์เตอร์',
                evidence: ev, kb: 'battery-match' };

            return { level: 'ok',
                title: 'แบตเตอรี่จ่ายไฟได้เต็มพิกัดของอินเวอร์เตอร์',
                detail: 'คายประจุได้จริง ' + kw(r.disLimit) + ' kW ชาร์จได้จริง ' + kw(r.chgLimit) + ' kW',
                evidence: ev, kb: 'battery-match' };
        }
    }
